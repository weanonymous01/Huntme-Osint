'use client';
// Updated navigation header without Blog
		...
		>
			<nav className="mx-auto flex h-14 w-full max-w-5xl items-center justify-between px-4">
				<div className="flex items-center gap-5">
					<a href="/" className="hover:bg-accent/50 rounded-md px-2 py-1 flex items-center font-bold text-xl text-white tracking-tight transition-colors">
						<span>Huntme</span>
					</a>
							...
							<NavigationMenuItem>
								...
								<NavigationMenuContent className="bg-background p-1 pr-1.5 pb-1.5">
									<div className="grid w-[520px] grid-cols-2 gap-2">
										<ul className="bg-popover space-y-1.5 rounded-md border p-2 shadow">
										...
										<ul className="space-y-1.5 p-2">
											{companyLinks2.map((item, i) => (
												<li key={i}>
													<NavigationMenuLink
														href={item.href}
														className="flex p-2 hover:bg-accent flex-row rounded-md items-center gap-x-2 text-sm text-foreground"
													>
			...
			<MobileMenu open={open} className="flex flex-col justify-between gap-2 overflow-y-auto">
				<NavigationMenu className="max-w-full">
					<div className="flex w-full flex-col gap-y-2">
						...
						<a
							href="/pricing"
							className="flex p-2 hover:bg-accent rounded-md items-center gap-x-2 text-sm text-foreground font-medium"
						>
							Pricing
						</a>
					</div>
...
function ListItem({
	...
	return (
		<NavigationMenuLink
			className={cn(
				'w-full flex flex-row items-center gap-x-3 rounded-md p-2 hover:bg-accent focus:bg-accent transition-colors data-[active=true]:bg-accent/50',
				className
			)}
			{...props}
			asChild
		>
			<a href={href} className="flex flex-row items-center gap-x-3 w-full">
				<div className="bg-background/50 flex aspect-square size-10 shrink-0 items-center justify-center rounded-md border shadow-xs">
					<Icon className="text-foreground size-5" />
				</div>
				<div className="flex flex-col items-start justify-center text-left min-w-0">
					<span className="font-medium text-sm text-foreground leading-snug">{title}</span>
					{description && (
...
const productLinks: LinkItem[] = [
	{
		...
		description: 'Lookup names, emails, carrier & social records',
		icon: PhoneCall,
	},
	{
		title: 'Vehicle Number OSINT',
		href: '/products/vehicle-osint',
		description: 'License plate & VIN public registration lookup',
		icon: Car,
	},
	{
		title: 'AI Case Solver',
		href: '/products/ai-case-solver',
		description: 'Connect related entities & auto-generate reports',
		icon: BrainCircuit,
	},
	{
		title: 'Investigation Reports',
		href: '/products/investigation-reports',
		description: 'Structured timelines, AI analysis & PDF export',
		icon: FileText,
	},
	{
		title: 'REST API',
		href: '/products/rest-api',
		description: 'Developer API with standardized JSON responses',
		icon: CodeIcon,
	},
	{
		title: 'Enterprise Infra',
		href: '/products/enterprise-infrastructure',
		description: 'Sub-100ms latency, 99.99% SLA & SOC-2 compliance',
		icon: Shield,
	},
];
...
const companyLinks: LinkItem[] = [
	{
		title: 'About Huntme',
		href: '/company/about',
		description: 'Our mission for lawful AI-powered OSINT',
		icon: Users,
	},
	{
		title: 'Target Users',
		href: '/target-users',
		description: 'Built for investigators, SOC teams & researchers',
		icon: Star,
	},
	{
		title: 'Use Cases',
		href: '/use-cases',
		description: 'Fraud research, due diligence & threat hunting',
		icon: Handshake,
	},
];
